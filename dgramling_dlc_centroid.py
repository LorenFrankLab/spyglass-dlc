...
class DLCCentroid(dj.Computed):
    ...
    def make(self, key):
        ...
        centroid_method = params["params"].pop("centroid_method")
        bodyparts_avail = cohort_entries.fetch("bodyparts")
        # TODO, generalize key naming
        if centroid_method == "four_led_centroid":
            if "green_LED" in params["params"]:
                assert (
                    params["params"]["green_LED"] in bodyparts_avail
                ), f'{params["params"]["green_LED"]} not a bodypart used in this model'
            else:
                raise ValueError(
                    "A green led needs to be specified for the 4 led centroid method"
                )
            if "red_LED_L" in params["params"]:
                assert (
                    params["params"]["red_LED_L"] in bodyparts_avail
                ), f'{params["params"]["red_LED_L"]} not a bodypart used in this model'
            else:
                raise ValueError(
                    "A left red led needs to be specified for the 4 led centroid method"
                )
            if "red_LED_C" in params["params"]:
                assert (
                    params["params"]["red_LED_C"] in bodyparts_avail
                ), f'{params["params"]["red_LED_C"]} not a bodypart used in this model'
            else:
                raise ValueError(
                    "A center red led needs to be specified for the 4 led centroid method"
                )
            if "red_LED_R" in params["params"]:
                assert (
                    params["params"]["red_LED_R"] in bodyparts_avail
                ), f'{params["params"]["red_LED_R"]} not a bodypart used in this model'
            else:
                raise ValueError(
                    "A right red led needs to be specified for the 4 led centroid method"
                )
            bodyparts_to_use = [
                params["params"]["green_LED"],
                params["params"]["red_LED_L"],
                params["params"]["red_LED_C"],
                params["params"]["red_LED_R"],
            ]

        elif centroid_method == "two_pt_centroid":
            if "point1" in params["params"]:
                assert (
                    params["params"]["point1"] in bodyparts_avail
                ), f'{params["params"]["point1"]} not a bodypart used in this model'
            else:
                raise ValueError(
                    "point1 needs to be specified for the 2 pt centroid method"
                )
            if "point2" in params["params"]:
                assert (
                    params["params"]["point2"] in bodyparts_avail
                ), f'{params["params"]["point2"]} not a bodypart used in this model'
            else:
                raise ValueError(
                    "point2 needs to be specified for the 2 pt centroid method"
                )
            bodyparts_to_use = [params["params"]["point1"], params["params"]["point2"]]
            ...
            bodyparts_to_use = [params["params"]["point1"]]
        ...
        centroid = centroid_func(pos_df, **params["params"])
...
def four_led_centroid(pos_df, **params):
    ...
    centroid_dict = {}
    centroid_dict["position_x"] = centroid[0]
    centroid_dict["position_y"] = centroid[1]
    return centroid_dict


def two_pt_centroid(pos_df, **params):
    """Determines the centroid of 2 points."""
    idx = pd.IndexSlice
    centroid = np.zeros(shape=(len(pos_df), 2))
...
def one_pt_centroid(pos_df, **params):
    idx = pd.IndexSlice
    PT1 = params.pop("point1", None)
